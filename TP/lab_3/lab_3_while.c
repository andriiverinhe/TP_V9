...
int main() {
  ...
  // значения x Цикл выполняется, пока x находится в интервале (-1.5, 1.5) и
  // флаг err равен 0
  while ((x - 1.5 < EXP && x + 1.5 > EXP) && !err) {
    if (x <= 0) {
      // Для x <= 0 используется формула ((x^2 - 2.0 * x^3) * cos(x^2))
      printf("f(%.2lf): %.2lf\n", x,
    ...
    x += h; // Увеличиваем x на значение h
  }
